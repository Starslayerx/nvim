...
8. **快速内容选择**: 使用 `<CR>`/`<BS>`/`<TAB>` 进行渐进式代码选择
9. **行内诊断**: 在代码行内显示诊断信息
10. **窗口选择器**: 快速切换和管理窗口
11. **快捷键提示**: 实时显示可用快捷键
12. **终端内图片显示**: 支持在终端中直接显示图片
...
1. 确保已安装 Neovim (推荐 0.10+)
2. 禁用不需要的 Perl 和 Ruby 提供商 (已在 init.lua 中配置)
3. 将此配置克隆到 `~/.config/nvim/`
4. 打开 Neovim，让 lazy.nvim 自动安装所有插件
5. 使用 `<leader>?` 查看可用快捷键

### 注意事项
- 配置已禁用 Perl 和 Ruby 语言服务器以提高启动速度
- 首次启动时会自动安装 treesitter parsers 和 LSP 服务器
- 透明度效果会在颜色主题加载后自动应用
